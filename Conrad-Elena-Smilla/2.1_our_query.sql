select like_count.m_messageid, total_likes, foreign_likes, first_foreign_liker.p_firstname, first_foreign_liker.p_lastname from 
    ...
    inner join 
        (select DISTINCT ON (m_messageid) p_firstname, p_lastname, m_messageid from (select * from message where m_length > 100) messages
    ...
    on like_count.m_messageid=first_foreign_liker.m_messageid
