...
WITH messages AS (SELECT message.m_messageid, message.m_creatorid
     ...
     foreign_likes AS (SELECT m.m_messageid as fl_messageid, COUNT(l.l_messageid) as f_like_count
                       FROM likes as l
     ...
     all_likes as (SELECT m.m_messageid AS al_messageid, COUNT(l.l_messageid) AS al_count
                   ...
                   WHERE m.m_messageid = l.l_messageid
                   GROUP BY m.m_messageid)
SELECT m.m_messageid, al.al_count, fl.f_like_count
FROM messages as m,
     all_likes as al,
     foreign_likes as fl
WHERE m.m_messageid = al.al_messageid
  AND m.m_messageid = fl.fl_messageid
  AND 2.0 * fl.f_like_count >= al.al_count
ORDER BY 1.0 * fl.f_like_count / al.al_count DESC;
